...
void __init_or_cpufreq s5pc100_setup_clocks(void)
{
	...
	/* mod by scsuh to make mmc clock */
#if 1
	{
		u32 tmp;

		tmp = readl(S5P_CLK_SRC2) & ~(0x00000fff);
		writel(tmp | 0x00000111, S5P_CLK_SRC2);
		tmp = readl(S5P_CLK_DIV3) & ~(0x00000fff);
		writel(tmp | 0x00000222, S5P_CLK_DIV3);
	}
#endif
