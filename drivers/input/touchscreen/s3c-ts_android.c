...
#ifdef CONFIG_FB_S3C_AT070TN83 //7" LCD
#define ANDROID_TS_RESOLUTION_X    800 
#define ANDROID_TS_RESOLUTION_Y    480 
static int pointercal[7]={-19563, 32, 64967232, -82, 20384, -26678208, 65536};
#elif CONFIG_FB_S3C_WX4300F   //4.3" LCD
#define ANDROID_TS_RESOLUTION_X    480 
#define ANDROID_TS_RESOLUTION_Y    272 
static int pointercal[7]={-13631, -39, 42597568, 67, 10778, -13576048 ,65536};
#else 						  //5" LCD
#define ANDROID_TS_RESOLUTION_X    800 
#define ANDROID_TS_RESOLUTION_Y    480 
static int pointercal[7]={-19563, 32, 64967232, -82, 20384, -26678208, 65536};
 ...
 */
static char *s3c_ts_name = "S3C TouchScreen For Android";
...
static void touch_timer_fire(unsigned long data)
{
	...
	if (updown) 
	{
		if (ts->count) 
		{
			xtemp = ts->yp / ts->count;
        	ytemp = ts->xp / ts->count;  
#ifdef CONFIG_ANDROID_TOUCHSCREEN_S3C_DEBUG
        	printk("pre: xtemp, ytemp = %d, %d\n", xtemp, ytemp);
#endif
            x = ( pointercal[2] + pointercal[0]*xtemp + pointercal[1]*ytemp ) / pointercal[6];
            y = ( pointercal[5] + pointercal[3]*ytemp + pointercal[4]*ytemp ) / pointercal[6];
...
static char banner[] __initdata = KERN_INFO "S3C Touchscreen driver for Android, (c) 2008 Samsung Electronics\n";
...
MODULE_DESCRIPTION("S3C touchscreen driver for Android");
MODULE_LICENSE("GPL");
