...
static void s5m8751_client_dev_register(struct s5m8751 *s5m8751, const char *name, struct platform_device **pdev)
{
	int ret;

	*pdev = platform_device_alloc(name, -1);
	if (pdev == NULL) {
		dev_err(s5m8751->dev, "Failed to allocate %s\n", name);
		return;
	}
...
int s5m8751_device_init(struct s5m8751 *s5m8751, int irq, struct s5m8751_platform_data *pdata)
{
	u8 id;
	s5m8751->read_dev(s5m8751, S5M8751_CHIP_ID, sizeof(id), &id);

	if (id == 0x0)
		printk("\ns5m8751 device is ready!!\n");
	else
		printk("\ns5m8751 device is not ready!!\n");
	...
	//s5m8751_reg_write(s5m8751, S5M8751_BUCK2_V1_SET, 0x98);	// Low(default) : 1.1v
	//s5m8751_reg_write(s5m8751, S5M8751_BUCK2_V2_SET, 0x94);	// High : 1.0v
	
	s5m8751_client_dev_register(s5m8751, "s5m8751-codec", &(s5m8751->codec.pdev));
	return 0;
}
...
{
	platform_device_unregister(s5m8751->codec.pdev);
